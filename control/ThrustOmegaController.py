...
class ThrustOmegaController(BaseControl):
    ...
    def __init__(self,
        ...
        # self.D_COEFF_OMEGA_TOR = np.array([100., 100., 100.])
        self.D_COEFF_OMEGA_TOR = np.array([0., 0., 0.])

        self.P_COEFF_FOR = np.array([.4, .4, 1.25])
        self.I_COEFF_FOR = np.array([.05, .05, .05])
        self.D_COEFF_FOR = np.array([.2, .2, .5])
        self.P_COEFF_TOR = np.array([7000., 7000., 60000.])
        self.I_COEFF_TOR = np.array([.0, .0, 500.])
        self.D_COEFF_TOR = np.array([2000., 2000., 12000.])
    ...
    def computeControlFromInput(self, u, control_timestep, cur_pos, cur_quat, cur_ang_vel, target_pos, target_rpy):
        '''

        :param u: [1d thrust, wx, wy, wz]
        :return: action: [rpm1, rpm2, rpm3, rpm4]
        '''

        self.control_counter += 1
        pos_e = target_pos - cur_pos
        # possibly revisit, but this doesn't work because as we approach the goal very small changes
        # in position cause huge angle errors

        # target_z_ax = (pos_e) / np.linalg.norm(pos_e)  # unit vector in the direction of position error
        ...
        #                                   u[1:]
        #                                   )
        rpm, omega_e = self.omega_PID(control_timestep, pwm_thrust, cur_quat, cur_ang_vel, target_rpy, u[1:])
        cur_rpy = p.getEulerFromQuaternion(cur_quat)
        return rpm, pos_e, target_rpy[2] - cur_rpy[2], omega_e
    ...
    def omega_PID(self,
        ...
        omega_rate_e = np.zeros(3) - (cur_omega - self.last_omega) / control_timestep #target angular accel is always 0 for now
        ...
        rpy_rates_e = target_omega - (cur_rpy - self.last_rpy) / control_timestep
        self.last_rpy = cur_rpy
        self.integral_rpy_e = self.integral_rpy_e - rot_e * control_timestep
        self.integral_rpy_e = np.clip(self.integral_rpy_e, -1500., 1500.)
        self.integral_rpy_e[0:2] = np.clip(self.integral_rpy_e[0:2], -1., 1.)
        #### PID target torques ####################################
        target_torques_rpy = - np.multiply(self.P_COEFF_TOR, rot_e) \
                         + np.multiply(self.D_COEFF_TOR, rpy_rates_e) \
                         + np.multiply(self.I_COEFF_TOR, self.integral_rpy_e)


        target_torques = target_torques_omega #+ target_torques_rpy #balance between nice hover and reaching target omega
        target_torques = np.clip(target_torques, -3200, 3200)
        pwm = thrust + np.dot(self.MIXER_MATRIX, target_torques)
        pwm = np.clip(pwm, self.MIN_PWM, self.MAX_PWM)
        return self.PWM2RPM_SCALE * pwm + self.PWM2RPM_CONST, omega_e
    def calc_target_euler(self, cur_pos, cur_quat, cur_vel, target_pos, target_rpy, target_vel):
        cur_rotation = np.array(p.getMatrixFromQuaternion(cur_quat)).reshape(3, 3)
        pos_e = target_pos - cur_pos
